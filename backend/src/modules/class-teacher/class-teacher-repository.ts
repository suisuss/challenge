...
interface UpdateClassTeacherPayload {
  id: number | string;
  className: string;
  section: string;
  teacher: number;
...
const getClassTeacherById = async (id: number | string): Promise<any> => {
  const query = `
        SELECT
            id,
            class_name AS class,
...
const updateClassTeacherById = async (payload: UpdateClassTeacherPayload): Promise<number> => {
...
const findAllTeachers = async (): Promise<any[]> => {
  ...
  const { rows } = await processDBRequest<any>({ query, queryParams });
  return rows;
};
