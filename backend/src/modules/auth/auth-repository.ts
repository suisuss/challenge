...
export const findUserByUsername = async (username: string, client: PoolClient): Promise<any> => {
    const query = "SELECT * FROM users WHERE email = $1";
    const { rows } = await client.query(query, [username]);
    return rows[0];
};

export const invalidateRefreshToken = async (token: string): Promise<number | null> => {
...
export const getMenusByRoleId = async (roleId: number, client: PoolClient): Promise<any[]> => {
...
export const getRoleNameByRoleId = async (id: number, client: PoolClient): Promise<string> => {
...
export const saveUserLastLoginDate = async (userId: number, client: PoolClient): Promise<void> => {
    const now = new Date();
...
export const deleteOldRefreshTokenByUserId = async (userId: number, client: PoolClient): Promise<void> => {
    const query = `DELETE FROM user_refresh_tokens WHERE user_id = $1`;
    const queryParams = [userId];
    await client.query(query, queryParams);
};

export const isEmailVerified = async (id: number): Promise<boolean> => {
...
export const verifyAccountEmail = async (id: number): Promise<any> => {
    const query = `
        UPDATE users
...
export const doesEmailExist = async (id: number, email: string): Promise<any> => {
...
interface SetupUserPasswordPayload {
    userId: number;
    userEmail: string;
    password: string;
}

export const setupUserPassword = async (payload: SetupUserPasswordPayload): Promise<number | null> => {
