...
export const login = async (username: string, passwordFromUser: string): Promise<{
    accessToken: string;
    refreshToken: string;
    csrfToken: string;
    accountBasic: object;
...
export const getNewAccessAndCsrfToken = async (refreshToken: string): Promise<{
    accessToken: string;
    csrfToken: string;
    message: string;
}> => {
...
export const processAccountEmailVerify = async (id: number): Promise<{ message: string }> => {
...
export const processPasswordSetup = async (payload: PasswordSetupPayload): Promise<{ message: string }> => {
...
export const processResendEmailVerification = async (userId: number): Promise<{ message: string }> => {
    try {
...
export const processResendPwdSetupLink = async (userId: number): Promise<{ message: string }> => {
    try {
        const user = await findUserById(userId);
        if (!user) {
            throw new ApiError(404, USER_DOES_NOT_EXIST);
...
export const processPwdReset = async (userId: number): Promise<{ message: string }> => {
    try {
        const user = await findUserById(userId);
        if (!user) {
            throw new ApiError(404, USER_DOES_NOT_EXIST);
