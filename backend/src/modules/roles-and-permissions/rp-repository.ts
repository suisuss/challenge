...
const enableOrDisableRoleStatusByRoleId = async (id: string | number, status: boolean): Promise<number> => {
...
const switchUserRole = async (userId: string | number, newRoleId: string | number): Promise<number> => {
...
const checkPermission = async (roleId: string | number, apiPath: string, apiMethod: string): Promise<number> => {
    const query = `
        SELECT 1
        FROM permissions p
        JOIN access_controls ac ON p.access_control_id = ac.id
